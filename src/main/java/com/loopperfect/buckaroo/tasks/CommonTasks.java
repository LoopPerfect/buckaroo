...
public final class CommonTasks {
    ...
    public static Single<RecipeUnlocked> readRecipeUnlockedFile(final Path path) {
        Preconditions.checkNotNull(path);

        return Single.fromCallable(() -> {
            try {
                return Either.orThrow(Serializers.parseRecipeUnlocked(EvenMoreFiles.read(path)));
            } catch (final Throwable error) {
                throw ReadRecipeFileException.wrap(path, error);
            }
        }).subscribeOn(Schedulers.io());
