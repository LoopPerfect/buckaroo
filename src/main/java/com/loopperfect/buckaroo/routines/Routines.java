...
public final class Routines {
    ...
    private static String buckRepositoryName(final RecipeIdentifier identifier) {
        Preconditions.checkNotNull(identifier);
        return identifier.project.name;
    }

    private static String buckRepositoryPath(final RecipeIdentifier identifier) {
        Preconditions.checkNotNull(identifier);
        return "./buckaroo/" + identifier.project + "/" + identifier.version + "/";
    }

    private static String buckTarget(final RecipeIdentifier identifier) {
        Preconditions.checkNotNull(identifier);
        return buckRepositoryName(identifier) + "//" + identifier.project; // TODO: Allow user to override the name
    }

    public static String buckarooDeps(final ImmutableMap<Identifier, SemanticVersion> versions) {
        Preconditions.checkNotNull(versions);
        final String list = versions.entrySet().stream()
                .map(x -> RecipeIdentifier.of(x.getKey(), x.getValue()))
                .map(Routines::buckTarget)
                .map(x -> "  '" + x + "', \n")
                .collect(Collectors.joining(""));
        return "# Generated by Buckaroo, do not edit! \nBUCKAROO_DEPS = [\n" + list + "]\n";
    }

    static String buckConfig(final ImmutableMap<Identifier, SemanticVersion> versions) {
        Preconditions.checkNotNull(versions);
        final String list = versions.entrySet().stream()
                .map(x -> RecipeIdentifier.of(x.getKey(), x.getValue()))
                .map(x -> "  " + buckRepositoryName(x) + " = " + buckRepositoryPath(x))
                .collect(Collectors.joining("\n"));
        return "[repositories]\n" + list + "\n"; // TODO: Parse and amend a .buckconfig
    }
    ...
    public static final IO<Optional<IOException>> checkout(final Path path, final GitCommit commit) {
    ...
    private static String dependencyPath(final RecipeIdentifier recipeIdentifier) {
        Preconditions.checkNotNull(recipeIdentifier);
        return "//buckaroo/" +
                recipeIdentifier.project.name + "/" +
                recipeIdentifier.version.toString() + ":" +
                recipeIdentifier.project;
    }
                ...
                project -> {
                    final Either<IOException, ImmutableList<Recipe>> loadRecipesResult = loadRecipes
                            ...
                            .rightProjection(x -> x.stream()
                                    .flatMap(y -> y.join(z -> Stream.empty(), Stream::of))
                                    .collect(ImmutableList.toImmutableList()));
                    return loadRecipesResult.join(error -> {
                        ...
                        final Either<IOException, String> buckFile = BuckFile.generate(
                                ...
                                resolvedDependencies.entrySet()
                                        .stream()
                                        .collect(ImmutableMap.toImmutableMap(Map.Entry::getKey, x -> x.getValue().get())));
